@extends('layouts.app')

@section('content')
<div class="container">
    <h1 class="my-4">Detalhes do Usuário</h1>

    @if(session('success'))
        <div class="alert alert-success">{{ session('success') }}</div>
    @endif

    @if(session('error'))
        <div class="alert alert-danger">{{ session('error') }}</div>
    @endif

    {{-- Limite de empréstimos --}}
    @php
        $limiteMaximo = 5;
        $emprestimosAtivos = App\Models\Borrowing::where('user_id', $user->id)
                                ->whereNull('returned_at')
                                ->count();
        $percentual = ($emprestimosAtivos / $limiteMaximo) * 100;
    @endphp

    <div class="card mb-4">
        <div class="card-header">{{ $user->name }}</div>
        <div class="card-body">
            <p><strong>Email:</strong> {{ $user->email }}</p>
            <p><strong>Papel:</strong> 
                <span class="badge bg-{{ $user->role == 'admin' ? 'danger' : ($user->role == 'bibliotecario' ? 'warning' : 'secondary') }}">
                    {{ ucfirst($user->role) }}
                </span>
            </p>

            {{-- DÉBITO --}}
            @if($user->hasDebit())
                <div class="alert alert-danger mt-3">
                    <i class="bi bi-exclamation-triangle-fill"></i> 
                    <strong>Débito pendente:</strong> {{ $user->getDebitFormatted() }}
                    @if(auth()->user()->canManageBooks())
                        <a href="{{ route('debits.show', $user) }}" class="btn btn-sm btn-warning float-end">
                            <i class="bi bi-cash"></i> Registrar Pagamento
                        </a>
                    @endif
                </div>
            @else
                <div class="alert alert-success mt-3">
                    <i class="bi bi-check-circle-fill"></i> 
                    <strong>Nenhum débito pendente.</strong>
                </div>
            @endif

            <hr>
            <div class="row">
                <div class="col-md-6">
                    <p><strong>📚 Empréstimos ativos:</strong> {{ $emprestimosAtivos }} de {{ $limiteMaximo }}</p>
                </div>
                <div class="col-md-6">
                    <div class="progress" style="height: 20px;">
                        <div class="progress-bar bg-{{ $emprestimosAtivos >= $limiteMaximo ? 'danger' : ($emprestimosAtivos >= 4 ? 'warning' : 'success') }}" 
                             role="progressbar" 
                             style="width: {{ $percentual }}%;" 
                             aria-valuenow="{{ $emprestimosAtivos }}" 
                             aria-valuemin="0" 
                             aria-valuemax="{{ $limiteMaximo }}">
                            {{ number_format($percentual, 0) }}%
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <!-- Histórico de Empréstimos -->
    <div class="card">
        <div class="card-header">Histórico de Empréstimos</div>
        <div class="card-body">
            @if($user->books->isEmpty())
                <p>Este usuário não possui empréstimos registrados.</p>
            @else
                <table class="table">
                    <thead>
                        <tr>
                            <th>Livro</th>
                            <th>Data de Empréstimo</th>
                            <th>Data de Devolução</th>
                            <th>Status</th>
                            <th>Ações</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach($user->books as $book)
                            @php
                                $borrowedAt = \Carbon\Carbon::parse($book->pivot->borrowed_at);
                                $returnedAt = $book->pivot->returned_at ? \Carbon\Carbon::parse($book->pivot->returned_at) : null;
                                $diasEmprestimo = $returnedAt ? $borrowedAt->diffInDays($returnedAt) : $borrowedAt->diffInDays(now());
                                $diasAtraso = max(0, $diasEmprestimo - App\Models\User::DIAS_EMPRESTIMO);
                                $multa = $diasAtraso * App\Models\User::MULTA_POR_DIA;
                            @endphp
                            <tr>
                                <td>
                                    <a href="{{ route('books.show', $book->id) }}">{{ $book->title }}</a>
                                </td>
                                <td>{{ $book->pivot->borrowed_at }}</td>
                                <td>{{ $book->pivot->returned_at ?? 'Em Aberto' }}</td>
                                <td>
                                    @if(is_null($book->pivot->returned_at))
                                        <span class="badge bg-warning">📖 Emprestado</span>
                                    @elseif($multa > 0)
                                        <span class="badge bg-danger">⚠️ Devolvido com atraso</span>
                                    @else
                                        <span class="badge bg-success">✅ Devolvido no prazo</span>
                                    @endif
                                </td>
                                <td>
                                    @if(is_null($book->pivot->returned_at))
                                        <form action="{{ route('borrowings.return', $book->pivot->id) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                            <button class="btn btn-warning btn-sm">Devolver</button>
                                        </form>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
            @endif
        </div>
    </div>

    <a href="{{ route('users.index') }}" class="btn btn-secondary mt-3">
        <i class="bi bi-arrow-left"></i> Voltar
    </a>
</div>
@endsection