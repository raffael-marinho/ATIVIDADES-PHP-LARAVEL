                            ...
                            @foreach($users as $user)
                                @php
                                    $emprestimosAtivos = App\Models\Borrowing::where('user_id', $user->id)
                                                            ->whereNull('returned_at')
                                                            ->count();
                                    $temDebito = $user->hasDebit();
                                @endphp
                                <option value="{{ $user->id }}" {{ $temDebito ? 'disabled' : '' }}>
                                    {{ $user->name }} 
                                    ({{ $emprestimosAtivos }}/5 empréstimos ativos)
                                    @if($temDebito) 
                                        🚫 Débito: {{ $user->getDebitFormatted() }}
                                    @endif
                                </option>
                            @endforeach
                        </select>
                        <small class="text-muted">Usuários com débito não podem fazer empréstimos.</small>
                    </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach($book->users as $user)
                            @php
                                $borrowedAt = \Carbon\Carbon::parse($user->pivot->borrowed_at);
                                $returnedAt = $user->pivot->returned_at ? \Carbon\Carbon::parse($user->pivot->returned_at) : null;
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    @elseif($multa > 0)
                                        <span class="badge bg-danger">⚠️ Multa: {{ $user->getDebitFormatted() }}</span>
                                    @else
